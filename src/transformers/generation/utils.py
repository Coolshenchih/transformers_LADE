...
class GenerationMixin:
    ...
    def _validate_model_kwargs(self, model_kwargs: Dict[str, Any]):
        ...
        if "win" in model_kwargs:
            model_args |= {'win'}
        if "lev" in model_kwargs:
            model_args |= {"lev"}
        if "guess" in model_kwargs:
            model_args |= {"guess"}
    ...
    def generate(
        ...
        win: Optional[int] = None, # 8803 LADE
        lev: Optional[int] = None, # 8803 LADE
        guess: Optional[int] = None, # 8803 LADE
        **kwargs,
        ...
        generation_mode = generation_config.get_generation_mode(assistant_model, secondary_assistant_model, win)
        ...
        if generation_mode == GenerationMode.LADE:
            # 11. run greedy search
            
            result = self._jacobi_greedy_search_multilevel(
                input_ids,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
    ...
    def _jacobi_greedy_search_multilevel(
        self,
        input_ids: torch.LongTensor,
        ...
        **model_kwargs,
        ...
        ############### configurations 
        WINDOW_SIZE = win
        GUESS_SET_SIZE = guess
        ALWAYS_FWD_ONE = 1
        LEVEL = lev
        DEBUG = 0
        DIST_WORKERS = 1
        LOCAL_RANK = 0
        USE_FLASH = 0 #not use flash by default
        POOL_FROM_PROMPT = 0
